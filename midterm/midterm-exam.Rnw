...
for(rho_sim in rho[1]){
  for(beta_p_sim in beta_p[1]){
    ...
    bias_vector = solve(t(X_minusp) %*% X_minusp) %*% t(X_minusp) %*% X[, p+1] %*% beta_p_sim
    
    print(dim(t(X_minusp)))
    print(dim(X_minusp))
    print(dim(X[, p+1]))
    print(dim(solve(t(X_minusp) %*% X_minusp)))
    print(dim(solve(t(X_minusp) %*% X_minusp) %*% t(X_minusp)))
    print(dim(solve(t(X_minusp) %*% X_minusp) %*% t(X_minusp) %*% X[, p+1]))
    print(dim(solve(t(X_minusp) %*% X_minusp) %*% t(X_minusp) %*% X[, p+1] %*% beta_p_sim))
    ...
    sim_result <- 
      data.frame(beta_p=beta_p_sim, rho=rho_sim, sim_bias=sim_bias, sim_type_i=sim_type_i, theor_bias=bias_beta_p_1)
...
bias_plt <- 
  sim_results %>%
    ggplot() +
    geom_point(aes(x = beta_p, y = sim_bias, color=as.factor(rho))) +
    # Add y = theoretical bias
    geom_line(aes(x = beta_p, y = theor_bias, color=as.factor(rho))) +
    ylab(TeX("Simulated bias ($n = 1,000$ draws)")) +
    xlab(TeX("$\\beta_p$")) +
    labs(color=TeX("$\\rho$"))
