...
robust_f_test %>%
  mutate(
    `Residual DF` = as.character(signif(`Residual DF`, 2)),
    DF = as.character(signif(DF, 2)),
    F = as.character(signif(F, 2)),
    `Pr(>F)` = as.character(signif(`Pr(>F)`, 2))) %>%
  kableExtra::kable(format = "latex", booktabs = TRUE, digits = 4, escape=FALSE) %>%
  kableExtra::save_kable("figures/robust_f_test.png")
@

Note that the only significant coefficients after multiple testing corrections are for state indicator variables, indicating that the model does fit significantly better when the indicator is included.

\begin{table}[h!]
\centering
\includegraphics[width = .6\textwidth]{figures/sig_p_val_tbl.png}
\caption{Summary of features significant with multiple testing corrections.}
\label{tab:sig_p_val_tbl}
\end{table}
