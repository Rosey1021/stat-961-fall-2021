...
for(rho_sim in rho){
  for(beta_p_sim in beta_p){
    ...
    #for(i in 1:1){
      ...
      type_i_error <- 
        pnorm(q = z_1_alpha - t_p_1/beta_hat_p_1 * bias_beta_p_1 * beta_p_sim, 
              lower.tail=FALSE)
...
\begin{figure}[h!]
\centering
\includegraphics[width = .6\textwidth]{figures/se_inflation_hist.png}
