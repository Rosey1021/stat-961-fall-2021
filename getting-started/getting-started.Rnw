...
\item Open RStudio and navigate to the \texttt{stat-961-fall-2021} directory in the RStudio ``Files'' pane (Figure~\ref{fig:rstudio}F). Click \texttt{More} -> \texttt{Set As Working Directory} (Figure~\ref{fig:rstudio}E). Open the Terminal in RStudio (Figure~\ref{fig:rstudio}C), click the down arrow, and click \texttt{Go to Current Directory}. (For Mac/Linux users, alternatively you can open a Terminal window outside of RStudio and navigate to the \texttt{stat-961-fall-2021} directory).
...
<<eval=FALSE>>=
git commit -am "[commit message briefly describing the changes]"
@  
at the Terminal.
\item \textbf{Submit.}
\end{itemize}
...
To complete this homework use the following steps:


\begin{enumerate}
  \item Locate and open the file \texttt{homework-0/homework-0.Rnw} in the files pane (Figure~\ref{fig:rstudio}F). 
