...
\begin{itemize}
\item Group structure: We have 10 samples $(\bm x_{i*}, y_i)$ each from 100 schools. 
...
\begin{enumerate}
\item Augment the regression model with extra terms. For example, we may want to include an indicator for school in the first example from Section~\ref{sec:origin-correlated-errors}. Using a fixed effect for each level of the grouping variable may use up too many degrees of freedom, so we can use \textit{random effects} instead. Hopefully we'll have time to cover random/mixed effects models in Unit 6.
...
\subsection{Robust standard error computation}

\subsubsection{Huber-White sandwich estimator} \label{sec:huber-white}
