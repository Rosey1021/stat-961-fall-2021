...
\begin{equation*}
\text{SSE}(\bm X_{*S}, \bm y) = \|\bm y -  \bm X_{*S}\bm{\widehat \beta}_S\|^2 \leq  \|\bm y -  \bm X\bm{\widehat \beta}\|^2 = \text{SSE}(\bm X, \bm y).
\end{equation*}
...
The simplest case to analyze is when a groups of predictors $S \subset \{1, \dots, p\}$ (suppose without loss of generality that $S = \{1, \dots, s\}$ for some $1 \leq s < p$) is orthogonal to the rest of the predictors in the sense that.
\begin{equation}
\bm X_{*S}^T \bm X_{*,\text{-}S} = \bm 0.
\end{equation}
In this case, we can derive the least squares coefficient vector $\bm{\widehat \beta} = (\bm{\widehat \beta}_S, \bm{\widehat \beta}_{\text{-S}})$ from the normal equations:
\begin{equation}
{\bm{\widehat \beta}_S \choose \bm{\widehat \beta}_{\text{-}S}} =  \bm{\widehat \beta} = (\bm X^T \bm X)^{-1}\bm X^T \bm y = 
\begin{pmatrix}
\bm X_S^T \bm X_S & 0 \\
0 & \bm X_{\text{-}S}^T \bm X_{\text{-}S}
\end{pmatrix}^{-1}{\bm X_S^T \choose \bm X_{\text{-}S}}\bm y = {(\bm X_S^T \bm X_S)^{-1}\bm X_S^T\bm y \choose (\bm X_{\text{-}S}^T \bm X_{\text{-}S})^{-1}\bm X_{\text{-}S}^T\bm y}.
\label{eq:orthogonality}
\end{equation}
Therefore, the least squares coefficients when regressing $\bm y$ on $(\bm X_S, \bm X_{\text{-}S})$ are the same as those obtained from regressing $\bm y$ separately on $\bm X_S$ and $\bm X_{\text{-}S}$.

\paragraph{Least squares estimates in the non-orthogonal case.}

Let's now focus our attention on a single predictor $x_j$. If this predictor is orthogonal to the remaining predictors, then the result~\eqref{eq:orthogonality} states that $\widehat \beta_j$ in the full regression can be obtained from simply regressing $y$ on $x_j$. However, this is usually not the case. Usually, $\bm x_{*j}$ has a nonzero projection onto $C(\bm X_{*,\text{-}j})$:
\begin{equation}
\bm x_{*j} = \bm X_{*,\text{-}j}\widehat \gamma + \bm x^{\perp}_{*j},
\end{equation}
