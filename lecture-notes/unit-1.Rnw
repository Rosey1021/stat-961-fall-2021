...
scots_races
@
\noindent Recall we had run a linear regression of \texttt{time} on \texttt{distance} and \texttt{climb}:
<<>>=
lm_fit = lm(time ~ distance + climb, data = scots_races)
summary(lm_fit)
@
...
@
\noindent The estimate stays the same but the standard error increases. Why is this the case? To obtain the variance inflation factors defined in equation~\eqref{eq:vif}, we can use the \verb|vif| function from the \verb|car| package:
<<>>=
car::vif(lm_fit)
@
