...
Let's go back to the \texttt{ScotsRaces} data from before:
<<>>=
scots_races
@
Recall we had run a linear regression of \texttt{time} on \texttt{distance} and \texttt{climb}:
<<>>=
lm_fit = lm(time ~ distance + climb, data = scots_races)
summary(lm_fit)
@
\noindent We get a coefficient of 6.35 with standard error 0.347 for \verb|distance|, where the standard error is an estimate of the quantity~\eqref{eq:conditional-variance}.
...
\noindent We can construct sum-of-squares decompositions~\eqref{eq:pythagorean-theorem} using the \verb|anova| function. This function takes as arguments the partial model and the full model. For example, consider the partial model \texttt{time ~ distance}.
<<>>=
...
\paragraph{Verifying the adjustment formula.} We can also test the adjustment formula~\eqref{eq:orthogonal-univariate} numerically. Let's consider the coefficient of \verb|distance| in the regression \verb|time ~ distance + climb|. We can obtain this coefficient by first regressing \verb|climb| out of \verb|distance| and \verb|time|:
<<>>=
...
scots_races_resid = 
  bind_cols(scots_races, 
            dist_residuals = lm_dist_on_climb$residuals,
            time_residuals = lm_time_on_climb$residuals)

lm_adjusted = lm(time_residuals ~ dist_residuals-1, 
                 data = scots_races_resid)
summary(lm_adjusted)
@
\noindent We find a coefficient of 6.35 with standard error 0.347, which matches that obtained in the original regression. Suppose we had not regressed \verb|climb| out of \verb|time|:
<<>>=
lm_dist_adjusted = lm(time ~ dist_residuals-1, 
                 data = scots_races_resid)
summary(lm_dist_adjusted)
@
\noindent The estimate stays the same but the standard error increases. Why is this the case?
