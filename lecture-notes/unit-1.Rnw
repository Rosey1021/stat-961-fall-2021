...
For now, we can extract a few consequences of the relationship~\eqref{eq:pythagorean-theorem}. 

\begin{itemize}
\item TSS = SSR + SSE, $R^2$, and what happens as you add more predictors
