...
\begin{enumerate}
  ...
  \end{align*}
  Note that as $n \rightarrow \infty$, this quantity tends toward $|S| F$. That is to say, asymptotically $2 \log\Lambda$ is within a factor $|S|$ of $F$ (and thus, $\frac{2}{|S|} \log\Lambda$ is asymptotically $F$-distributed with $|S|$ and $n-p$ degrees of freedom).
...
So, $(\frac{n-p}{p-1}) \frac{R^2}{1-R^2} = \frac{||(\bm{H} - \bm{H}_1) \bm{y}||^2 / (p-1)}{||(\bm{I} - \bm{H}) \bm{y}||^2 / (n - p)} =: F$. But this is exactly the $F$-statistic for the hypothesis test with null hypothesis $H_0:\, \beta_1 = \dots = \beta_{p-1} = 0$. We can also invert this relationship and find that for $c := \frac{p-1}{n-p} F$, we have $R^2 = \frac{c}{c+1}$.
...
The plots of the empirical distributions of $R^2$ can be found in Figure~\ref{fig:R_2_plt}. Note that as $p$ increases, the mass of the empirical null distribution of $R^2$ slowly shifts to the right, going from right-skewed, to relatively symmetric, then left-skewed. When $p \approx 1$, we have that $c \approx 0$, so $R^2 \approx 0$, whereas when $p \approx n$, we have $c \approx F$, so $R^2 \approx 1$.
...
\[t^2 = \frac{\widehat{\bm{\beta}}^2_j s_j^2}{||\bm{y} - \bm{X} \widehat{\bm{\beta}}||^2 / (n-p)}\] and thus it suffices to show that 
\[\widehat{\bm{\beta}}^2_j s^2_j = ||\bm{X} \widehat{\bm{\beta}} - \bm{X}_{-j} \widehat{\bm{\beta}}_{-j}||^2 = ||(\bm{H} - \bm{H}_{-j}) \bm{y}||^2 = ||\bm{H}_j \bm{y}||^2 = ||\widehat{\beta}_{j | -j} \bm{x}_{*j}^{\bot}||^2.\]
Note that $s_j^2 = ||\bm{x}_{*j}^{\bot}||^2$, so this equivalent to proving that 
...
% NOTE: NEED TO FINISH
\item[(d)] We first recall that \[t(\bm{c}) = \frac{\bm{c}^T \widehat{\bm{\beta}}}{\widehat{\sigma} \sqrt{\bm{c}^T (\bm{X}^T \bm{X})^{-1} \bm{c}}} \sim t_{n - p}\]
under the null hypothesis $H_0: \bm{c}^T \bm{\beta}_{-0} = 0$. Taking $A := (\bm{X}^T \bm{X})^{-1}$ and recognizing that $\widehat{\sigma}$ is independent of $\bm{c}$, we have that
\begin{align*}
  c_{\max} :=& \argmax_{c \in \mathbb{R}^{p-1}} t(\bm{c}) \\
  =& \argmax_{c \in \mathbb{R}^{p-1}} \frac{\bm{c}^T \widehat{\bm{\beta}}}{\widehat{\sigma} \sqrt{\bm{c}^T (\bm{X}^T \bm{X})^{-1} \bm{c}}} \\
  =& \argmax_{c \in \mathbb{R}^{p-1}} \frac{\bm{c}^T \widehat{\bm{\beta}}}{\sqrt{\bm{c}^T \bm{A} \bm{c}}}.
\end{align*}
Note that this objective function is in fact scale invariant: if $\bm{d} := a \bm{c}$, then \[\frac{\bm{d}^T \widehat{\bm{\beta}}}{\sqrt{\bm{d}^T \bm{A} \bm{d}}} = \frac{(a \bm{c})^T \widehat{\bm{\beta}}}{\sqrt{(a \bm{c})^T \bm{A} (a \bm{c})}} = \frac{\bm{c}^T \widehat{\bm{\beta}}}{\sqrt{\bm{c}^T \bm{A} \bm{c}}}.\]
So, we have that we can rewrite the optimization problem as follows:
\[c_{\max} = \argmax_{c \in \mathbb{R}^{p-1}} \frac{\bm{c}^T \widehat{\bm{\beta}}}{\sqrt{\bm{c}^T \bm{A} \bm{c}}} = \argmax_{c \in \mathbb{R}^{p-1} \text{ s.t. } \bm{c}^T \bm{A} \bm{c} = 1} \bm{c}^T \widehat{\bm{\beta}}.\]

We can solve this problem using Lagrange multipliers and end up with a system of equations:
\begin{equation*}
  \begin{cases}
    \bm{0} = \frac{\partial}{\partial \bm{c}}[\bm{c}^T \widehat{\bm{\beta}} - \lambda (\bm{c}^T \bm{A} \bm{c} - 1)] \\
    1 = \bm{c}^T \bm{A} \bm{c}
  \end{cases} 
  = 
  \begin{cases}
    \bm{0} = \widehat{\bm{\beta}} - \lambda 2 \bm{A} \bm{c} \\
    1 = \bm{c}^T \bm{A} \bm{c}.
  \end{cases}
\end{equation*}

Solving, we get that $\lambda = \frac{1}{2} ||\bm{X} \widehat{\bm{\beta}}||$ and $\bm{c}_{\max} = \frac{\bm{A}^{-1} \widehat{\bm{\beta}}}{||\bm{X} \widehat{\bm{\beta}}||} = \frac{\bm{X}^T \bm{X} \widehat{\bm{\beta}}}{||\bm{X} \widehat{\bm{\beta}}||}$. 

So,
\begin{align*} 
  \bm{c}_{\max}^T \widehat{\bm{\beta}} &= \bigg( \frac{\bm{X}^T \bm{X} \widehat{\bm{\beta}}}{||\bm{X} \widehat{\bm{\beta}}||} \bigg)^T \widehat{\bm{\beta}} \\
  &= \frac{\widehat{\bm{\beta}}^T \bm{X}^T \bm{X} \widehat{\bm{\beta}}}{||\bm{X} \widehat{\bm{\beta}}||} \\
  &= \frac{(\bm{X} \widehat{\bm{\beta}})^T \bm{X} \widehat{\bm{\beta}}}{||\bm{X} \widehat{\bm{\beta}}||} \\
  &= ||\bm{X} \widehat{\bm{\beta}}||.
\end{align*}
Likewise, we have $\bm{c}_{\max}^T (\bm{X}^T \bm{X})^{-1} \bm{c}_{\max} = 1$ by construction. So, $t_{\max} = \frac{||\bm{X} \widehat{\bm{\beta}}||}{\widehat{\sigma}} = \frac{||\bm{X} \widehat{\bm{\beta}}|| / \sqrt{1}}{||\bm{y} - \bm{X} \widehat{\bm{\beta}}|| / \sqrt{n-p}}$ and
\[t_{\max}^2 = \frac{||\bm{X} \widehat{\bm{\beta}}||^2 / 1}{||\bm{y} - \bm{X} \widehat{\bm{\beta}}||^2 / (n-p)}.\]
...
crime_data_full <-
  ...
  select(-NAME) %>%
  # Create violent crime rate variable
  mutate(CrimeRate = Violent/Pop)

head(crime_data_full)
@

% NOTE: TO FINISH, ADD WORD SUMMARIES
\item[(b)]
<<hw 3b, echo=TRUE, results='hide'>>=
...
crime_data_full %>%
  select(CrimeRate, Metro, HighSchool, Poverty) %>%
  GGally::ggpairs(columnLabels = c("Crime Rate", "Metro", "High School", "Poverty"))
dev.off()
...
colnames(summary_stats) <- 
  c("CrimeRate", "Metro", "HighSchool", "Poverty")

summary_stats <-
  summary_stats %>%
  # Get rid of added text from summary()
  mutate(CrimeRate = 
           str_replace(CrimeRate, pattern = ".*:", replacement = ""),
         Metro = 
           str_replace(Metro, pattern = ".*:", replacement = ""),
         HighSchool = 
           str_replace(HighSchool, pattern = ".*:", replacement = ""),
         Poverty = 
           str_replace(Poverty, pattern = ".*:", replacement = ""))

rownames(summary_stats) <- c("Min.", "Q1", "Median", "Mean", "Q3", "Max.")
...
We plot the correlation between our variables in Figure~\ref{fig:corr_plt}, pairwise scatterplots in Figure~\ref{fig:pairs_plt}, and summary statistics for each variable in Table~\ref{tab:crime_tbl}.
...
\begin{table}[h!]
\centering
\includegraphics[width = .3\textwidth]{figures/conf_int_tbl.png}
