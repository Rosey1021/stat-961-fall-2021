...
Note that we have that the sample correlation between $\bm{x}_{*1}$ and $\bm{x}_{*2}$ is roughly $3/5 = 0.60$, the correlation between $x_1$ and $x_2$ calculated in (c). Likewise, the sample correlation between $\widehat{\beta}_1$ and $\widehat{\beta}_2$ is roughly $-3/5 = -0.60$, which was predicted as the correlation between $\widehat{\beta}_1$ and $\widehat{\beta}_2$ in (c).
