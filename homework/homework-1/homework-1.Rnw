...
\begin{sol}
% Solution goes here
\item[(a)] Note that the model \[y = \beta_0 + \beta_1 x_1 + \beta_2 x_2 + \epsilon,\ \epsilon \sim (0, \sigma^2)\] is equivalent to the model \[y = \beta_0' + \beta_1(x_1 - \overline{x}_1) + \beta_2(x_2 - \overline{x}_2),\ \epsilon \sim (0, \sigma^2),\text{ with }\beta_0' = \beta_0 + \beta_1 \overline{x}_1 + \beta_2 \overline{x}_2.\]
So, the coefficients $\widehat{\beta}_1$, $\widehat{\beta}_2$ are unaffected by mean centering. Note that the regression $y = \beta_1 (x_1 - \overline{x}_1) + \beta_2 (x_2 - \overline{x}_2)$ has the same coefficients of this second model by orthogonality; $\overline{x}_1$ is the projection of $x_1$ onto 1 and $\overline{x}_2$ is the projection of $x_2$ onto 1, so $x_1 - \overline{x}_1, x_2 - \overline{x}_2 \perp 1$.
...
where $\bm{\Sigma}'$ is the covariance matrix for $\widehat{\beta}_1$ and $\widehat{\beta}_2$ under the model $y = \beta_1(x_1 - \overline{x}_1) + \beta_2(x_2 - \overline{x}_2)$.
