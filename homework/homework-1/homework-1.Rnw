...
In our case, $p = 2$ and $\hat{\mu}_i = \overline{y}_{C(i)}$, so the numerator is the SSW. Thus, an unbiased estimator is $s^2 = \frac{SSW}{n - 2} = \frac{3910.742}{70} = 55.8676$.
